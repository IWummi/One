...
public class EventLogPanel extends JPanel implements ConnectionListener, MessageListener, ActionListener {
	...
	/**
	 * Number of events -setting id ({@value}). Defines the number of events to
	 * show in the panel.
	 */
	 ...
	 * 
	 * @param gui
	 *            The where this log belongs to (for callbacks)
	 */
	 ...
	 * @param host1
	 *            Host that caused the event or null if there was not any
	 * @param host2
	 *            Another host that was involved in the event (or null)
	 * @param message
	 *            Message that was involved in the event (or null)
	 * @param highlight
	 *            If true, the log entry is highlighted
	 */
	private void addEvent(String description, DTNHost host1, DTNHost host2, Message message, boolean highlight) {
		...
		eventPanes.add(0, eventPane); // adds events at top of the box
		// old version eventPanes.add(eventPane);

		// if the log is full, remove oldest entries first
		if (this.eventPanes.size() > maxNrofEvents) {
			eventPanes.remove(this.eventPanes.size() - 1);// deletes last entry
															// obv. highest is
															// last one
															// :eventPanes.remove(0);
		}
	...
	/**
	 * Adds a new button to a log entry panel and attaches a client property
	 * into it
	 * 
	 * @param panel
	 *            Panel where to add the button
	 * @param o
	 *            Client property object to add
	 * @param clientProp
	 *            Client property key to use for the object
	 */
	 ...
	 * 
	 * @param check
	 *            EventLogControls used to check if this entry type should be
	 *            shown and/or paused upon
	 * @param name
	 *            Text description of the event
	 * @param host1
	 *            First host involved in the event (if any, can be null)
	 * @param host2
	 *            Second host involved in the event (if any, can be null)
	 * @param message
	 *            The message involved in the event (if any, can be null)
	 */
